...
//				globs.knxmonitor.registerGA(this.groupAddress.address, this.update.bind(this));
				...
				if (this.service.handler === "Default") {
					...
					for (var ibGA = 0; ibGA < this.ListenGroupAddressList.length; ibGA++) {
						...
						this.globs.knxmonitor.registerGA(cBGA.address, function(val, src, dest, type) {
							knxAccess.writeValueHK(val, type, this.getDPT(), cBGA.reverse);
						}.bind(this));
					}
				} else {
					// bind to custom API
					for (var isga = 0; isga < this.listenGroupAddresses.length; isga++) {
						var thisGA = this.listenGroupAddresses[isga];
						this.globs.knxmonitor.registerGA(thisGA, function(val, src, dest, type) {
							this.service.customAPI.knxbusEventCatcher (name, val, src, dest, type);
						}.bind(this));	
					}
	...
	defaultUpdateKNXValue(value, callback, context) {
		...
		for (var int2 = 0; int2 < this.setGroupAddressList.length; int2++) {
			writeValueKNX(value, this.setGroupAddressList[int2], callback);
		}
