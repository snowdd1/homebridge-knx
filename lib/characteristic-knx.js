			...
			if (this.config.Listen) {
				...
				if (this.service.handler === "Default") {
					...
					for (var ibGA = 0; ibGA < this.listenGroupAddressList.length; ibGA++) {
						...
						this.globs.knxmonitor.registerGA(cBGA.address, this.getDPT(), function (val, src, dest) {
					...
					for (var isga = 0; isga < this.listenGroupAddressList.length; isga++) {
						...
						this.globs.knxmonitor.registerGA(thisGA.address, this.getDPT(), function (val, src, dest) {
							this.service.customServiceAPI.knxbusEventCatcher(this.name, val, src, dest);
						}.bind(this));	
					}
