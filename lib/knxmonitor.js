...
/**
 * Opens a KNX group socket for bi-directional communication and calls the callback function upon success  
 * 
 * @param {object} opts - Obejct containing the net.socket connection information, such as {host:host, port:port}
 * @param {function} callback - function(parser) that is called upon success
 * @returns void
 */
...
function groupsocketlisten(opts, callback) {
	...
	conn.socketRemote(opts, function(err) {
		...
		conn.on('close', function() {
			running = false;
			console.log("INFO Connection to knxd lost, trying to reconnect...");
			startMonitor(opts);
		});
	});
}
...
/**
 * Registers a single knx group address in the array of subscriptions.
 * Does not bind the address to event yet
 * @param {String} groupAddress - KNX group address string, must be validated before
 * @param {function} callback - function(value, src, destGA, DPT) that is called when address is receipt from the KNX bus 
 */
