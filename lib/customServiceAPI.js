...
class customServiceAPI  {
		...
		if (listenGroupAddresses) {
			...
			for (let isga = 0; isga < listenGroupAddresses.length; isga++) {
				...
				this.serviceKNX.globs.knxmonitor.registerGA(thisGA, function(val, src, dest, type) {
					this.knxbusEventCatcher (name, val, src, dest, type, dptype);
				}.bind(this));	
			}
	 ...
	 * @private
	 */
	knxbusEventCatcher (characteristicName, val, src, dest, type, dpt) {
		val = KNXAccess.decodeKNXValue(val, dpt);	// Decode KNX Value based on DPT
