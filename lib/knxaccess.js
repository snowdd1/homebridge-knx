...
/** Opens a connection, if none is open for writing
 *  Pooling addresses enhancement #78
 */
function knxConnection(callback) {
	
    globs.info('In knxConnection...');
	if (connOpen) {
		// is already open, use it
	    globs.info('knxConnection: using existing connection.');
		
		callback(null, knxdConnection);
		
		
	} else if (connOpening) {
		// the connection is in statu nascendi (waiting for callback)
		// wait another 1000ms for the recursive call
		globs.info('Already opening knxd connection, waiting 1000ms...');
		setTimeout(function() {
			knxConnection(callback);
		}, 1000);
		
	} else {
		// try to open a new one
		connOpening = true; // indicate connection in progress
		knxdConnection = new knxd.Connection();
		globs.info("DEBUG in knxaccess.js:knxConnection: created empty connection, trying to connect socket to " + globs.knxd_ip + ":" + globs.knxd_port);
		knxdConnection.socketRemote({
			host : globs.knxd_ip,
			port : globs.knxd_port
		}, function(err) {
			// this is the onConnect event from socket.connect(opts, callback)
			if (err) {
			...
			connOpening = false;
			connOpen = true;
			knxdConnection.on('close', function() {
				connOpen = false;
				knxdConnection = null;
				console.log('knxd connection closed by server. Next write attempt will try reconnecting');
			});
			globs.info('knxConnection: new Connection successfully established.');
			callback(null,knxdConnection);
		});
	}
}
...
allFunctions.knxwrite = function(callback, groupAddress, dpt, value) {
	...
	knxConnection(function(err, knxdConnection) {
		if (err) {
			// a fatal error occurred
			console.error("FATAL: knxd or eibd not reachable: " + err);
			throw new Error("Cannot reach knxd or eibd service, please check installation and configuration .json");
		...
		knxdConnection.openTGroup(dest, 1, function(err) {
			if (err) {
				...
				}, true); // doNotCloseConnection=true;
			}
		});
	});
};
...
allFunctions.writeValueHK = function(val, chrKNX, type, reverse) {
	...
	case globs.Characteristic.Formats.UINT8:
		...
		if (characteristic.props.minValue == undefined && characteristic.props.maxValue == undefined) {
			...
			returnValue = safeSet(characteristic, val);
		} else {
	...
	if (returnValue!==null) {
		...
		if (characteristic.value!==returnValue) {
			// value has changed
			globs.info('Value changed, updating homebridge');
			characteristic.updateValue(returnValue, undefined, 'fromKNXBus');
		} else {
			globs.info('INFO HomeKit:  No value change');
		}
	} else {
		globs.info('INFO HomeKit:  No valid value.');
	}
	globs.info('exiting writeValueHK()');
};
