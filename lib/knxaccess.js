...
allFunctions.setUInt32 = function(value, callback, gaddress) {

        var numericValue = 0;
        if (value && value >= 0 && value <= 4294967295) {
                numericValue = value; // assure 0..4294967295 for KNX bus
        }
        globs.debug("setUInt32: Setting " + gaddress + " int to " + value + " (" + numericValue + ")");
        allFunctions.knxwrite(callback, gaddress, 'DPT12', numericValue);

};

allFunctions.setInt32 = function(value, callback, gaddress) {

        var numericValue = 0;
        if (value && value >= -2147483647 && value <= 2147483647) {
                numericValue = value; // assure -2147483647..2147483647 for KNX bus
        }
        globs.debug("setInt32: Setting " + gaddress + " int to " + value + " (" + numericValue + ")");
        allFunctions.knxwrite(callback, gaddress, 'DPT13', numericValue);

};

allFunctions.setFloat32 = function(value, callback, gaddress) {

        var numericValue = 0;
        if (value) {
                numericValue = value; // homekit expects precision of 1 decimal
        }
        globs.debug("setFloat32: Setting " + gaddress + " Float to %s", numericValue);
        allFunctions.knxwrite(callback, gaddress, 'DPT1xx4', numericValue);
...
*/
	...
	case "DPT5":
		...
		break;
	case "DPT12":
		allFunctions.setUInt32(value, callback, setGA);
		break;
	case "DPT13":
		allFunctions.setInt32(value, callback, setGA);
		break;
	case "DPT14":
		allFunctions.setFloat32(value, callback, setGA);
		break;
...
allFunctions.writeValueHK = function(val, chrKNX, type, reverse) {
	...
	val = allFunctions.decodeKNXValue(val, chrKNX.getDPT());
...
*/
			...
			if (chrKNX.name == 'CurrentTemperature' || chrKNX.name == 'eveCurrentTemperature') {
				globs.debug('@@@@@@@@@@@@ KNX FakeGatoHistoryService: Sending data for CurrentTemperature');
				logData.temp        = returnValue; // Used for Eve Weather & Eve Room
				logData.currentTemp = returnValue; // Used for Eve Thermo
			}

			// Prepare Data for "Temperature"
			if (chrKNX.name == 'TargetTemperature' || chrKNX.name == 'eveTargetTemperature') {
				globs.debug('@@@@@@@@@@@@ KNX FakeGatoHistoryService: Sending data for SetTemperature');
				logData.setTemp = returnValue; 		// Used for Eve Thermo
			}
			...
			// Prepare Data for "Aqua" - This is not working yet. Mainly due to missing implementation in Fakegato History
...
allFunctions.decodeKNXValue = function(val, dpt) {
	if (typeof(val) == 'object') {
		switch(dpt) {
			case 'DPT12':
				val = val.readUInt32BE(0);
				globs.debug("decodeKNXValue: Got Object as value for " + dpt + " decoding UInt32");
				break;
			case 'DPT13':
				val = val.readInt32BE(0);
				globs.debug("decodeKNXValue: Got Object as value for " + dpt + " decoding Int32");
				break;
			case 'DPT14':
				val = val.readFloatBE(0);
				globs.debug("decodeKNXValue: Got Object as value for " + dpt + " decoding Float32");
				break;
			default:
				globs.debug("decodeKNXValue: Got Object as value for " + dpt + " - unhandled DPT - assuming Float32");
				val = val.readFloatBE(0);
				break;
		}
	}
