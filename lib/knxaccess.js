			...
			if (err) {
				...
				knxdConnection.sendAPDU(msg, function(err) {
					if (err) {
						...
						globs.info("knxAccess.knxwrite: knx data sent: Value " + value + " for GA " + groupAddress);
						if (callback) {
							callback();
						}
					}
...
allFunctions.setBooleanState = function(value, callback, gaddress, reverseflag) {
	...
	globs.info("setBooleanState: Setting " + gaddress + " " + reverseflag ? " (reverse)" : "" + " Boolean to " + numericValue);
...
allFunctions.setPercentage = function(value, callback, gaddress, reverseflag) {
	...
	globs.info("setPercentage: Setting " + gaddress + " percentage " + reverseflag ? " (reverse)" : "" + "  to " + value + " (" + numericValue + ")");
...
allFunctions.setInt = function(value, callback, gaddress) {
	...
	globs.log("setInt: Setting " + gaddress + " int to " + value + " (" + numericValue + ")");
	allFunctions.knxwrite(callback, gaddress, 'DPT5', numericValue);
...
allFunctions.setFloat = function(value, callback, gaddress) {
	...
	globs.log("setFloat: Setting " + gaddress + " Float to %s", numericValue);
...
allFunctions.setHVACState = function(value, callback, gaddress) {
	...
	globs.log("setHVAC: Setting " + gaddress + " HVAC to %s", KNXvalue);
...
allFunctions.writeValueKNX = function(value, groupAddress, callback) {
	...
	case "DPT5":
		...
		break;
	default: 
		globs.log( "[ERROR] unknown type passed: [" + groupAddress.dptype + "]" );
...
allFunctions.writeValueHK = function(val, chrKNX, type, reverse) {
	...
	globs.info("knxAccess.writeValueHK("+ val +","+chrKNX.name+","+type+","+reverse + ")");
	globs.info("knxAccess.writeValueHK: Format "+ characteristic.props.format +"");
	...
	case globs.Characteristic.Formats.BOOL:
		...
		globs.info("BOOL:[" + characteristic.displayName + "]: Received value from bus:" + val + " of type " + type );
	...
	case globs.Characteristic.Formats.UINT8:
		...
		globs.info("INT:[" + characteristic.displayName + "]: Received value from bus:" + val + " of type " + type );
		if (characteristic.props.minValue === undefined && characteristic.props.maxValue === undefined) {
			...
			if (val >= (characteristic.props.minValue || 0) && val <= (characteristic.props.maxValue || 255)) {
				...
				globs.log('error', "["  + "]:[" + characteristic.displayName + "]: Value %s out of bounds %s...%s ", val,
						(characteristic.props.minValue || 0), (characteristic.props.maxValue || 255));
			}
		}

		break;
	case globs.Characteristic.Formats.FLOAT:
		globs.info("FLOAT:[" + characteristic.displayName + "]: Received value from bus:" + val + " of type " + type + " for " +
		...
		if (validValue) {
			...
			globs.log(":[" + characteristic.displayName + "]: Value %s out of bounds %s...%s ", hk_value, characteristic.props.minValue,
					characteristic.props.maxValue);
		}
		break;
	default:
		globs.info("knxAccess.writeValueHK() - NO KNOWN TYPE");
		break;
	}
};
