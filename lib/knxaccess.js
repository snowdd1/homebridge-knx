...
KNXAccess.prototype = {
		...
		knxwrite: function(callback, groupAddress, dpt, value) {
			...
			knxdConnection.socketRemote({ host: this.globs.knxd_ip, port: this.globs.knxd_port }, function(err) {
				if (err) {
				...
				knxdConnection.openTGroup(dest, 1, function(err) {
					if (err) {
						this.globs.log("[ERROR] knxwrite:openTGroup: " + err);
						callback(err);
					} else {
						// this.globs.log("DEBUG opened TGroup ");
						var msg = knxd.createMessage('write', dpt, parseFloat(value));
						knxdConnection.sendAPDU(msg, function(err) {
							if (err) {
								this.globs.log("[ERROR] knxwrite:sendAPDU: " + err);
								callback(err);
							} else {
								this.globs.log("knx data sent: Value "+value+ " for GA "+groupAddress);
								callback();
							}
						}.bind(this));
					}
...
//				this.globs.log(gaddress + " event ping pong, exit!");
				if (callback) {
					callback();
				}
			} else {
				...
				this.globs.log("["+ this.name +"]:Setting "+gaddress+" " + reverseflag ? " (reverse)":""+ " Boolean to %s", numericValue);
		...
		setPercentage: function(value, callback, gaddress, reverseflag) {
				...
				this.globs.log("["+ this.name +"]:Setting "+gaddress+" percentage to %s (%s)", value, numericValue);
		...
		setInt: function(value, callback,  gaddress) {
				...
				this.globs.log("["+ this.name +"]:Setting "+gaddress+" int to %s (%s)", value, numericValue);
				this.knxwrite(callback, gaddress,'DPT5',numericValue);
		...
		setFloat: function(value, callback,  gaddress) {
				...
				this.globs.log("["+ this.name +"]:Setting "+gaddress+" Float to %s", numericValue);
		...
		setHVACState: function(value, callback,  gaddress) {
				...
				this.globs.log("["+ this.name +"]:Setting "+gaddress+" HVAC to %s", KNXvalue);
		...
		knxread: function(groupAddress){
			...
			knxdConnection.socketRemote({ host: this.knxd_ip, port: this.knxd_port }, function(err) {
				if (err) {
					throw {name:"KNXD connection failed", message:"The connection to the knx daemon failed. Check IP and Port."};
				}
				var dest = knxd.str2addr(groupAddress);
				// this.log("DEBUG got dest="+dest);
				knxdConnection.openTGroup(dest, 1, function(err) {
