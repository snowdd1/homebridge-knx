...
allFunctions.validateAddressText = function(groupAddress) {
	...
	// assume triple notation (0..15) (0..7) (0..255)
	var addressArray = groupAddress.match(/^(([0-9]|[1-9][0-9]{1,2})\/([0-9]|[1-9][0-9]{1,2})\/([0-9]|[1-9][0-9]{1,2}))/);
	if (!addressArray) {
		// no valid structure
		return 'ERR no valid group address structure (15/7/255)';
	}
	if (parseInt(addressArray[2]) > 15) {
		return 'ERR no valid group address structure (15/7/255): first triple exceeds 15';
	}
	if (parseInt(addressArray[3]) > 7) {
		return 'ERR no valid group address structure (15/7/255): second triple exceeds 7';
	}
	if (parseInt(addressArray[4]) > 255) {
		return 'ERR no valid group address structure (15/7/255): third triple exceeds 255';
	}
	return 'OK';
};
