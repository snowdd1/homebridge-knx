 ...
 * 
 * Like the jalousie/blinds example just with reversed percentage 100% =
 */
'use strict';
/**
 * @type {HandlerPattern}
...
class GiraAwningActuator extends HandlerPattern {
	...
	onKNXValueChange(field, oldValue, knxValue) {
		...
		if (field==="TargetPosition") {
			...
			switch (knxValue) {
			case 0:
				this.myAPI.setValue("TargetPosition", 0); // in position, so home shows "closing"
				this.myAPI.setValue("PositionState", 0); //up
				break;
			case 1:
				this.myAPI.setValue("TargetPosition", 100); // out position, so home shows "opening"
...
module.exports=	GiraAwningActuator;
