	...
	onKNXValueChange(field, oldValue, knxValue) {
		...
		if (field==="KNXDoorClosed") {
			...
			if (knxValue === this.sensorOn && oldValue !== this.sensorOn) {
				...
				// Startup, KNXDoorClosed old== undef, new= 0 -> ?
				if (oldValue !== null){

					console.log('INFO: KNXDoorClosed = false detected');

				} else { // Init run

					console.log('INFO: Init Garage Door HK Status KNXDoorClosed = false');
			...
			if (knxValue === this.sensorOn && oldValue !== this.sensorOn) {
				// Status 0 (open) detected
					...
					if (oldValue !== null){
						...
						console.log('INFO: Init Garage Door HK Status KNXDoorOpen = true');
				...
				// Startup, KNXDoorOpen old== undef, new= 0 -> ?
				if (oldValue !== null){

					console.log('INFO: KNXDoorOpen = false detected');

				} else { // Init run

					console.log('INFO: Init Garage Door HK Status KNXDoorOpen = false');
