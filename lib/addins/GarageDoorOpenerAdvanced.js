	...
	onKNXValueChange(field, oldValue, knxValue) {
			...
			if (knxValue === 1) {
				...
				if (oldValue !== 1){ // value changed?
					...
					if (this.timer1 !== undefined){
						...
						this.timer1 = undefined;
						console.log('INFO ' + this.debugName + ': Timer für KNXDoorOpen reset');
					}
					...
					if (myCurrentDoorState === 1){ // IF DOOR IS CLOSED
						...
						if (this.simulateDoorContactMode==="knx" && this.doorContactPresent==="open"){
							...
							console.log('INFO ' + this.debugName + ': Timer für KNXDoorOpen started');
							this.timer1 = setTimeout (function () {
													console.log('INFO ' + this.debugName + ': Timeout, simulating Door Open');
													
													console.log('INFO ' + this.debugName + ': KNXDoorOpen = ' + this.sensorOn + ' set');
		...
		if (field === "KNXPulseDown") {
			...
			if (knxValue === 1 && oldValue !== 1) {
				...
				// INFO: It could happen that this timer is set twice in  case KNXDoorOpen get's in first,
		...
		if (field==="KNXDoorClosed") {
			...
			// value changed?
			if (knxValue === this.sensorOn && (oldValue !== this.sensorOn || oldValue === null)) {
				...
				if (oldValue === this.sensorOn){ // oldValue = sensorOn
					...
					if (!this.timer1) {
						if (this.simulateDoorContactMode==="knx" && this.doorContactPresent==="closed"){
							...
							// simulate this even on the bus
							this.myAPI.knxWrite("KNXDoorOpen", !this.sensorOn, "DPT1"); // Door not closed

							console.log('INFO ' + this.debugName + ': Timer für KNXDoorOpen started, timer to expire in = ' + this.doorRunTime);
							this.timer1 = setTimeout (function () {
...
// -> TargetDoorState needs to be set.
		...
		if (field==="KNXDoorOpen") {
			...
			console.log('INFO ' + this.debugName + ': In if-case -> field===KNXDoorOpen');
			...
			if (knxValue === this.sensorOn && (oldValue !== this.sensorOn || oldValue === null)) {
				...
				if (oldValue !== null){
					...
					console.log('INFO ' + this.debugName + ': KNXDoorOpen detected');

				} else { // Init run

					console.log('INFO ' + this.debugName + ': Init Garage Door HK Status KNXDoorOpen = true');
				...
				if (oldValue === this.sensorOn){ // oldValue == sensorOn
					...
					console.log('INFO ' + this.debugName + ': KNXDoorOpen = false detected');
					...
					if (!this.timer1) {
						...
						console.log('INFO ' + this.debugName + ': KNXDoorOpen = timer running detected');
					}

				} else if (oldValue === null) { // Init run

					console.log('INFO ' + this.debugName + ': Init Garage Door HK Status KNXDoorOpen = false');
