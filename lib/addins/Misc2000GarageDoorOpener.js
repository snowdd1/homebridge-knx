...
class Misc2000GarageDoorOpener extends HandlerPattern {
	...
	constructor(knxAPI) {
		...
		// Homekit and KNX Init
		// Sync at Start (Garage Door must be closed)
		// simulate 2nd contact sensor (open door) by software/timer
		// if you want ro simulate it set it to true if not to false
				...
				if (oldValue!==1){ // Nur wenn Wert sich geändert hat
					...
					// also CurrentDoorState = 1
						...
						if (this.simulateDoorOpenContact){
							...
							}.bind(this),20000, field);
						} // simulateDoorOpenContact
					} // ("KNX_Garagentor_zu")===1

					else if (this.myAPI.getValue("CurrentDoorState")===2){ // IF DOOR IS OPENING
					...
					else if (this.myAPI.getValue("CurrentDoorState")===3){ // IF DOOR IS CLOSING
						console.log('INFO: Aktueller Status CurrentDoorState = 3 CLOSING erkannt');
						console.log('INFO: CurrentDoorState = 4 STOPPED setzen');
						this.myAPI.setValue("CurrentDoorState", 4); // STOPPED
					} // ("CurrentDoorState")===2 //OPENING

					else if (this.myAPI.getValue("CurrentDoorState")===4){ // IF DOOR IS STOPPED
					...
					else if (this.myAPI.getValue("CurrentDoorState")===0){ // IF DOOR IS OPEN
