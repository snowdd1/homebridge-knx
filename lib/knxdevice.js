...
function KNXDevice(globs, config, platformAccessory) {
	...
	if (platformAccessory) {
		...
		this.config.Context = globs.API.hap.uuid.generate('KNX' + Math.random() + Math.random() + Math.random());
	...
	//plugin-2
	/******************  
	 *  The services are immediatly prepared, and not waited for getServices() to be called
	 */ 
	...
	// if everything setup properly, we can inject it into homekit (if it wasen't existing before)
