...
class KNXDevice {
	...
	constructor (globs, config, platformAccessory) {
		...
		if (platformAccessory) {
			...
			globs.debug('Iterating the passed context object:');
			iterate(this.platformAccessory.context);
			globs.debug('Iterating the passed context object END');
		...
		for (var int = 0; int < currServices.length; int++) {
			...
			if (myKNXService.failed) {
				...
				this.glob.errorlog("Homebridge couldn't create KNX service:" + configService.ServiceName);
			} else {
