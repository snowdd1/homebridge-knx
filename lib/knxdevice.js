...
class KNXDevice {
	...
	constructor (globs, config, platformAccessory) {
		...
		// Each homebridge device should have a unique device serial number - FakeGato History needs this to display the history properly
		this.SerialNumber = this.config.SerialNumber || "KNX-Device-" + this.config.UUID;
		...
		informationService.setCharacteristic(this.globs.Characteristic.Manufacturer, this.config.Manufacturer || "Opensource Community")
			...
			.setCharacteristic(this.globs.Characteristic.SerialNumber,this.SerialNumber);
		...
		// Remove old FakeGatoServices if restored from Cache
		this.globs.info('@@@@@@@@@@@@ KNX FakeGatoHistory: Checking for logging services restored from cache');
		for (var i = this.platformAccessory.services.length - 1; i > 0; i--) {
			this.globs.info('@@@@@@@@@@@@ KNX FakeGatoHistory: Found service "' + this.platformAccessory.services[i].displayName + '" with UUID "' + this.platformAccessory.services[i].UUID + '"');
			if (this.platformAccessory.services[i].UUID == 'E863F007-079E-48FF-8F27-9C2605A29F52') {
				this.globs.info('@@@@@@@@@@@@ KNX FakeGatoHistory: This is an old history service - removing this service');
				this.platformAccessory.removeService(this.platformAccessory.services[i]);
			}
		}
		...
		// Adding Fakegato History Object
		if (this.config.HistoryType !== undefined) {

			this.platformAccessory.log = this.globs.log;
	               	this.platformAccessory.loggingService = new this.globs.FakeGatoHistoryService(this.config.HistoryType, this.platformAccessory, {size: 365*24*6, storage:'fs'});
/*
console.log("@@@@@@@@ Config ****************************************************************************************************");
                        console.log(util.inspect(this.config, {depth: 4}));
console.log("@@@@@@@@ Created Logging Service ***********************************************************************************");
                        console.log(util.inspect(this.platformAccessory.loggingService, {depth: 4}));
console.log("@@@@@@@@ platformAccessory *****************************************************************************************");
                        console.log(util.inspect(this.platformAccessory, {depth: 4, showHidden: true}));
console.log("@@@@@@@@ ***********************************************************************************************************");
*/
			this.services.push(this.platformAccessory.loggingService);
		}
