 ...
 */
GroupAddress.prototype.complete(address, globs, characteristic){
	...
	if (!globs.config.GroupAddresses[address]) {
		...
		if (Characteristic.props.format === globs.hap.Characteristic.Formats.BOOL) {
			// Boolean
			return new GroupAddress(address, 'automatically generated', DPTTypes.DPT1, true, true, true, 'INITIAL')
		} else if (Characteristic.props.format === globs.hap.Characteristic.Formats.INT || Characteristic.props.format === globs.hap.Characteristic.Formats.UINT8) {
			if (Characteristic.props.unit === globs.hap.Characteristic.Units.PERCENTAGE) {
				// percentage
		...
		} else if (Characteristic.props.format === globs.hap.Characteristic.Formats.FLOAT) {
